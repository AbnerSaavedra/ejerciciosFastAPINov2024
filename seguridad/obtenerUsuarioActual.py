...
def dumb_decode_token(token):
    return User(username=token + "dummydecoded", email="john@example.com", full_name="John Doe", disabled=True)
...
async def get_current_user_active(current_user: Annotated[User, Depends(get_current_user)]):
    print("CU: ", current_user.disabled)
...
@app.get("/users/me")
async def read_users_me(current_user: Annotated[User, Depends(get_current_user_active)]):
    return current_user
